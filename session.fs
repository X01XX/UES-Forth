...
\ Return highest rate negative number, le 0, having an item
...
: session-find-highest-le-zero-rate-superset ( regc1 sess0 -- rate )
    ...
    while
        #3 pick                             \ | regc-lol-link rates-link regc1
        #2 pick link-get-data               \ | regc-lol-link rates-link regc1 regc-lst
        regioncorr-list-any-superset?       \ | regc-lol-link rates-link bool
        if
            link-get-data                   \ regc1 sess0 | regc-lol-link rate
            2nip nip                        \ rate
            \ space ." rate: " dup dec. cr
...
\ Its semi-random because next steps can be chosen based on having the needed changes,
...
\
\ Pseudo code
\   Is regc-from a subset regc-to? y -> Abort.
...
\           Failed? y -> done, fail.
\
\           Append returned regc-seq to return list.
\           Get return list last item, apply selected pathstep for next-from.
\       then
\
\       \ Check for success.
...
: session-calc-path-fc ( depth regc-to regc-from pthstp-lst sess0 -- regc-seq t | f )
    ...
    \ Prep for loop.
    #3 pick regioncorr-copy                 \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from'

    \ One RegionCorr step forward per cycle, except for recursion.
    begin
        \ Get next step(s) forward.

        \ cr ." begin: cur path: " over .regioncorr-list space ." cur-from: " dup .regioncorr cr
        \ cr .stack-gbl cr

        #5 pick                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' cur-to
        ...
        \ Select one.
        dup list-get-length                 \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstp-nxt-lst' len
        random                              \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstp-nxt-lst' inx
        ...
        if
            \ Calc next-from.
            over swap                           \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' cur-from' pthstpx
            pathstep-apply-to-regioncorr-fc     \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from', nxt-from' t | f
            false? abort" apply failed?"

            \ Store cur-from.
            swap                                \ depth regc-to regc-from pthstp-lst sess0 | ret-lst nxt-from' cur-from'
            #2 pick                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst nxt-from' cur-from' ret-lst
            list-push-end-struct                \ depth regc-to regc-from pthstp-lst sess0 | ret-lst nxt-from'

        else                                    \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx
            \ cr ." selected step: " dup .pathstep cr
            \ Prep for recursion.
            dup                                 \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx pthstpx
            pathstep-get-initial-regions        \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx regc-to
            #8 pick 1- swap                     \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx depth- regc-to
            #3 pick                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx depth- regc-to cur-from'
            #7 pick                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx depth- regc-to cur-from' pthstp-lst
            #7 pick                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx depth- regc-to cur-from' pthstp-lst sess0

            recurse                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx, regc-seq' t | f
            if                                  \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx regc-seq'
                \ Store regc-seq
                dup                             \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx regc-seq' regc-seq'
                #4 pick                         \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx regc-seq' regc-seq' ret-lst
                regioncorr-list-append          \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx regc-seq'
                regioncorr-list-deallocate      \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx
                \ Add selected step.
                swap regioncorr-deallocate      \ depth regc-to regc-from pthstp-lst sess0 | ret-lst pthstpx
                over list-get-last-item         \ depth regc-to regc-from pthstp-lst sess0 | ret-lst pthstpx last-item
                swap                            \ depth regc-to regc-from pthstp-lst sess0 | ret-lst last-item pthstpx
                pathstep-apply-to-regioncorr-fc \ depth regc-to regc-from pthstp-lst sess0 | ret-lst, nxt-from' t | f
                false? abort" apply failed?"
            else                                \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' pthstpx
                drop regioncorr-deallocate      \ depth regc-to regc-from pthstp-lst sess0 | ret-lst
                regioncorr-list-deallocate      \ depth regc-to regc-from pthstp-lst sess0
                2drop                           \ depth regc-to regc-from
                2drop                           \ depth
                drop
                false
                \ cr ." false exit 2: " cr
        ...
        if                                  \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from'
            \ Add cur-to to return list.
        ...
        if
            ...
            \ Add cur-from intersection to the return list.
            over                            \ depth regc-to regc-from pthstp-lst sess0 | ret-lst cur-from' ret-lst
            regioncorr-list-push-end        \ depth regc-to regc-from pthstp-lst sess0 | ret-lst

            \ Add regc-to intersection to the return list.
            #4 pick                         \ depth regc-to regc-from pthstp-lst sess0 | ret-lst regc-to
            regioncorr-copy                 \ depth regc-to regc-from pthstp-lst sess0 | ret-lst regc-to'
            over                            \ depth regc-to regc-from pthstp-lst sess0 | ret-lst regc-to' ret-lst
            regioncorr-list-push-end        \ depth regc-to regc-from pthstp-lst sess0 | ret-lst
...
: session-calc-path ( regc-to regc-from sess0 -- regc-seq t | f )
    ...
    session-find-highest-le-zero-rate-intersecting  \ regc-to regc-from sess0 | rate-to

    #2 pick #2 pick                             \ regc-to regc-from sess0 | rate-to regc-from sess0
    session-find-highest-le-zero-rate-superset  \ regc-to regc-from sess0 | rate-to rate-from
    ...
    \ The search is semi-random depth-first, so give it a few tries.
    \ Its semi-random because next steps can be chosen based on having the needed changes,
    \ and intersecting the current from regioncorr, or being reachable without additional
    \ needed changes.  Needing additional changes to reach a step indicates it would be premature.
...
: session-change-to-plans ( regc-to sess0 -- plnc-lst t | f )
    ...
    if
        ...
        \ cr ." at xx: " cr
        dup                                         \ sess0 regc-to regc-from' regc-seq' regc-seq'
        #3 pick                                     \ sess0 regc-to regc-from' regc-seq' regc-seq' regc-to
        #3 pick                                     \ sess0 regc-to regc-from' regc-seq' regc-seq' regc-to regc-from'
        #6 pick                                     \ sess0 regc-to regc-from' regc-seq' regc-seq' regc-to regc-from' sess0
        session-calc-plnclst-from-regc-lst          \ sess0 regc-to regc-from' regc-seq', plnc-lst t | f
        if
    ...
    2drop
    false
    \ cr ." session-change-to-plans: end: " cr
;
...
: session-process-need-list ( ned-lst1 sess0 -- bool )
    ...
    begin
        ...
        if
            ...
            session-find-highest-le-zero-rate-intersecting  \ ned-lst1 sess0 inx-lst' rnd-inx nedx sess0 regc-to' regc-from' rate-to

            over #4 pick                        \ ned-lst1 sess0 inx-lst' rnd-inx nedx sess0 regc-to' regc-from' rate-to regc-from' sess0
            session-find-highest-le-zero-rate-superset  \ ned-lst1 sess0 inx-lst' rnd-inx nedx sess0 regc-to' regc-from' rate-to rate-from
...
: session-do-dn-command ( cmd-lst1 sess0 -- )   \ Do the "dn" command. Do a need, given it's number in the displayed list.
    ...
    over list-get-second-item               \ cmd-lst1 sess0 tkn1

    cr ." You entered number " dup . cr

    \ Check lower bound.
    dup 0<                              \ cmd-lst1 sess0 n flag
    if
        cr ." Number entered is out of range" cr
        2drop drop
        exit
    then                                \ cmd-lst1 sess0 n

    \ Check higher bound.
    over                                \ cmd-lst1 sess0 n sess0
    session-get-needs                   \ cmd-lst1 sess0 n ned-lst
    dup list-get-length                 \ cmd-lst1 sess0 n ned-lst ned-len
    #2 pick                             \ cmd-lst1 sess0 n ned-lst ned-len n
    swap                                \ cmd-lst1 sess0 n ned-lst n ned-len
    >=
    if                                  \ cmd-lst1 sess0 n ned-lst flag
        cr ." Number entered is out of range" cr
        2drop 2drop
        exit
    then                                \ cmd-lst1 sess0 n ned-lst

    \ Get selected need.
    list-get-item                       \ cmd-lst1 sess0 ned
    cr ." You chose need: " dup .need cr
    swap                                \ cmd-lst1 ned sess0
    session-do-need                     \ cmd-lst1 bool
    2drop
;
...
: session-do-to-command ( regc-to1 sess0 -- )  \ Do the "to" command. Change state to a given regioncorr.
    ...
    #2 pick                                     \ regc-to1 sess0 regc-from' regc-to1
    over                                        \ regc-to1 sess0 regc-from' regc-to1 regc-from'
    regioncorr-superset?                        \ regc-to1 sess0 regc-from' bool
    if
    ...
    cr ." from: " dup .regioncorr space ." to: " #2 pick .regioncorr cr
    regioncorr-deallocate                       \ regc-to1 sess0
    2dup                                        \ regc-to1 sess0 regc-to1 sess0
    session-change-to-plans                     \ regc-to1 sess0, plnc-lst' t | f
    if
        cr ." Plan found: " dup .plancorr-list cr
        dup plancorr-list-run-plans             \ regc-to1 sess0 planc-lst' bool
        if
            cr ." Plan suceeded" cr
        else
            cr ." Plan failed" cr
        then
        plancorr-list-deallocate                \ regc-to1 sess0
        2drop
    else
        cr ." No plan found" cr
        2drop
...
: session-do-pd-command ( cmd-lst1 sess0 -- ) \ Do the "pd" command. Print a Domain.
    ...
    if
        \ Set current domain.
        tuck swap                           \ cmd-lst1 dom dom sess0
        session-set-current-domain          \ cmd-lst1 dom
        .domain                             \ tnk-lst1
        drop
    else
...
: session-do-psd-command ( cmd-lst1 sess0 -- ) \ Do the "pds" command. Print square detail for a domain's action.
    ...
    \ cr ." domain " dup . cr
    over session-find-domain                \ cmd-lst1 sess0, dom t | f
    if                                      \ cmd-lst1 sess0 dom
        \ Set current domain.
        dup #2 pick                         \ cmd-lst1 sess0 dom dom sess0
        session-set-current-domain          \ cmd-lst1 sess0 dom
    else
    ...
    \ Get action.
    #2 pick list-get-third-item             \ cmd-lst1 sess0 dom tkn2

    over                                    \ cmd-lst1 sess0 dom act-id dom
    domain-find-action                      \ cmd-lst1 sess0 dom, act t | f
    if
        tuck swap                           \ cmd-lst1 sess0 act act dom
        domain-set-current-action           \ cmd-lst1 sess0 act
        action-get-squares                  \ cmd-lst1 sess0 sqr-lst
        cr .square-list cr                  \ cmd-lst1 sess0
        2drop
        exit
    else
...
: session-do-tos-command ( cmd-lst1 sess0 -- ) \ Do the "tos" command. Change a domain to a state.
    ...
    if                                          \ cmd-lst1 sess0 dom
        \ Set current domain.
        2dup swap                               \ cmd-lst1 sess0 dom dom sess0
        session-set-current-domain              \ cmd-lst1 sess0 dom
    else
    ...
    \ Get state.
    #2 pick list-get-third-item                 \ cmd-lst1 sess0 dom tkn1

    dup is-value?                           \ cmd-lst1 sess0 dom to-sta bool
    if
        swap                                \ cmd-lst1 sess0 to-sta dom
        dup domain-get-current-state        \ cmd-lst1 sess0 to-sta dom cur-sta
        rot swap                            \ cmd-lst1 sess0 dom to-sta cur-sta
        2dup =
        if                                  \ cmd-lst1 sess0 dom to-sta cur-sta
        ...
        \ Do domain-get-plan
        dup region-new swap                 \ cmd-lst1 sess0 dom cur-reg' to-sta
        dup region-new swap                 \ cmd-lst1 sess0 dom to-reg' cur-reg'

        rot                                 \ cmd-lst1 sess0 to-reg' cur-reg' dom
        #2 pick swap                        \ cmd-lst1 sess0 to-reg' cur-reg' to-reg' dom
        #2 pick swap                        \ cmd-lst1 sess0 to-reg' cur-reg' to-reg' cur-reg' dom
        domain-get-plan                     \ cmd-lst1 sess0 to-reg' cur-reg', plan' t | f
        if                                  \ cmd-lst1 sess0 to-reg' cur-reg' plan'
            swap region-deallocate          \ cmd-lst1 sess0 to-reg' plan'
            swap region-deallocate          \ cmd-lst1 sess0 plan'
            dup                             \ cmd-lst1 sess0 plan' plan'
            plan-run                        \ cmd-lst1 sess0 plan' flag
            swap plan-deallocate            \ cmd-lst1 sess0 flag
            if
                cr ." Plan succeeded" cr
            else
                cr ." Plan failed" cr
            then
            2drop
        else                                \ cmd-lst1 sess0 to-reg cur-reg
            region-deallocate               \ cmd-lst1 sess0 to-reg
            region-deallocate               \ cmd-lst1 sess0
            2drop
            cr ." No plan found" cr
        then
    else
...
: session-do-scs-command ( cmd-lst1 sess0 -- ) \ Do the "scs" command.  Sample the current state of a domain.
    ...
    over domain-find-action                 \ cmd-lst1 sess0 dom, act t | f
    if
        swap 2dup                           \ cmd-lst1 sess0 act dom act dom
        domain-set-current-action           \ cmd-lst1 sess0 act dom
        dup domain-get-current-state        \ cmd-lst1 sess0 act dom cur-sta
        rot                                 \ cmd-lst1 sess0 dom cur-sta act
        action-get-sample                   \ cmd-lst1 sess0 dom smpl'
        dup sample-get-result               \ cmd-lst1 sess0 dom smpl' smpl-r
        rot                                 \ cmd-lst1 sess0 smpl' smpl-r dom
        domain-set-current-state            \ cmd-lst1 sess0 smpl'
        sample-deallocate                   \ cmd-lst1 sess0
        2drop
    else
...
: session-do-pa-command ( cmd-lst1 sess0 -- ) \ Do "pa" command. Print a domain's action.
    ...
    over domain-find-action                     \ cmd-lst1 sess0 dom, act t | f
    if                                          \ cmd-lst1 sess0 dom act
        tuck swap                               \ cmd-lst1 sess0 act act dom
        domain-set-current-action               \ cmd-lst1 sess0 act
        .action                                 \ cmd-lst1 sess0
        2drop
    else                                        \ dom
...
: session-do-sas-command ( cmd-lst1 sess0 -- ) \ Do the "sas" command. Take an arbitrary domain action for a given state.
    ...
    \ Get state.
    #3 pick list-get-fourth-item                \ cmd-lst1 sess0 act dom tkn2
    dup is-not-value?                           \ cmd-lst1 sess0 act dom sta flag
    if
    ...
    2dup swap                                   \ cmd-lst1 sess0 act dom sta sta dom
    domain-set-current-state                    \ cmd-lst1 sess0 act dom sta
    rot                                         \ cmd-lst1 sess0 dom sta act
    action-get-sample                           \ cmd-lst1 sess0 dom smpl'
    dup sample-get-result                       \ cmd-lst1 sess0 dom smpl' smpl-r
    rot                                         \ cmd-lst1 sess0 smpl' smpl-r dom
    domain-set-current-state                    \ cmd-lst1 sess0 smpl'
...
: session-eval-user-input ( cmd-lst1 sess0 -- bool )
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" pa" str=                         \ cmd-lst1 sess0 tkn0 bool
    if
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" to" str=                         \ cmd-lst1 sess0 tkn0 bool
    if
        \ Go to a given regioncorr.
        drop                                \ cmd-lst1 sess0
        ...
        dup region-list-corresponding?  \ sess0 reg-lst bool
        if
        else
            cr ." Did not understand regioncorr string" cr
            2drop
        ...
        \ Make regioncorr from region list.
        regioncorr-new              \ sess0 regc-to'

        \ Run command.
        tuck swap                   \ regc-to' regc-to' sess0
        session-do-to-command       \ regc-to'
        regioncorr-deallocate
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" dn" str=                         \ cmd-lst1 sess0 tkn0 bool
    if
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" pd" str=                         \ cmd-lst1 sess0 tkn0 bool
    if
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" cds" str=                        \ cmd-lst1 sess0 tkn0 bool
    if
        \ Print a domain, given a domain ID.
        drop                            \ cmd-lst1 sess0
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" psd" str=                        \ cmd-lst1 sess0 tkn0 bool
    if
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" tos" str=                        \ cmd-lst1 sess0 tkn0 bool
    if
        \ Print a domain, given a domain ID.
        drop                            \ cmd-lst1 sess0
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" scs" str=                        \ cmd-lst1 sess0 tkn0 bool
    if
        \ Print a domain, given a domain ID.
        drop                            \ cmd-lst1 sess0
    ...
    dup token-get-string                \ cmd-lst1 sess0 tkn0 c-addr u
    s" sas" str=                        \ cmd-lst1 sess0 tkn0 bool
    if
        \ Print a domain, given a domain ID.
        drop                            \ cmd-lst1 sess0
