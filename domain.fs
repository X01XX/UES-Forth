...
\ Give a planstep, known to have a needed change, check if it has:
\ 1) The planstep's initial region is a superset of reg-from or
\ 2) A rule from reg-from to the planstep's initial region does not require a needed change,
...
\ Give a planstep, known to have a needed change, check if it has:
\ 1) The planstep's result region is a superset of reg-to or
\ 2) A rule from the planstep's result region to regsto does not require a needed change,
\    which would make using it premature.
: domain-rule-valid-previous-step? ( cngs4 reg-to reg-from plnstp1 dom0 -- bool )
    ...
    over planstep-get-rule          \ cngs4 reg-to reg-from plnstp1 dom0 rul
    rule-calc-result-region         \ cngs4 reg-to reg-from plnstp1 dom0 rul-r'
    #4 pick                         \ cngs4 reg-to reg-from plnstp1 dom0 rul-r' reg-to
    over                            \ cngs4 reg-to reg-from plnstp1 dom0 rul-r' reg-to rul-r'
    region-intersects?              \ cngs4 reg-to reg-from plnstp1 dom0 rul-r' bool
    swap region-deallocate          \ cngs4 reg-to reg-from plnstp1 dom0 bool
    if
        2drop 2drop drop
        true
        \ cr ." domain-rule-valid-previous-step?: found direct planstep." cr
        exit
    then
    ...
    \ Get changes from rule reselt region to reg-to.
    #3 pick                         \ cngs4 reg-to reg-from plnstp1 dom0 reg-to
    #2 pick planstep-get-rule       \ cngs4 reg-to reg-from plnstp1 dom0 reg-to rul
    rule-calc-result-region         \ cngs4 reg-to reg-from plnstp1 dom0 reg-to rul-r'
    tuck                            \ cngs4 reg-to reg-from plnstp1 dom0 rul-r' reg-to rul-r'
    changes-new-region-to-region    \ cngs4 reg-to reg-from plnstp1 dom0 rul-r' cngs'
    swap region-deallocate          \ cngs4 reg-to reg-from plnstp1 dom0 cngs'
    ...
    if
        ...
        2drop 2drop drop
        true
        \ cr ." domain-rule-valid-previous-step?: found indirect planstep." cr
    then
;

\ Return a step backward, from a goal region,
\ towards a start region.
\ If more than one step is found, randomly choose one,
\ to support a random depth-first strategy.
: domain-calc-step-bc ( reg-to reg-from dom0 -- step t | f )
    ...
    while
        ...
        #3 pick link-get-data               \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk | cngs' reg-to reg-from plnstpx
        #8 pick                             \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk | cngs' reg-to reg-from plnstpx dom0
        domain-rule-valid-previous-step?    \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk | bool
        if
            dup link-get-data               \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk | plnstpx
            #2 pick                         \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk | plnstpx val-stps'
            list-push-struct                \ reg-to reg-from dom0 | spt-lst' cngs' val-stps' stp-lnk
   ...
   \ Check if any valid next steps found.
    ...
    over                                    \ reg-to reg-from dom0 | stp-lst2 inx stp-lst

    \ Extract step.
    planstep-list-remove-item               \ reg-to reg-from dom0 | stp-lst2 stpx

    \ Clean up.                             \ reg-to reg-from dom0 | stp-lst2 stpx
    swap planstep-list-deallocate           \ reg-to reg-from dom0 | stpx
